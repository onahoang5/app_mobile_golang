...
func GetRatingRestaurant(appCtx appctx.AppContext) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		repo := restaurantratingrepo.NewGetRatingRestaurantRepo(store)
		biz := restaurantratingbiz.NewGetRatingRestaurantBiz(repo)

		users, err := biz.GetRatingRestaurant(c.Request.Context(), &filter, &paging)
